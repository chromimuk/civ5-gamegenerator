...
(function () {
    init(civs, maps, victories);
})();

function init(civs, maps, victories) {
    printDate();
    let results = roll();
    showResults(results.civ, results.map, results.victory);
}
...
function printVolume(rollCiv, rollMap, rollVictory) {
...
function roll() {
    ...
    let rollCiv = getRandomInt(0, civs.length - 1);
    let rollMap = getRandomInt(0, maps.length - 1);
    let rollVictory = getRandomInt(0, victories.length - 1);
    printVolume(rollCiv, rollMap, rollVictory);

    return {
        civ: civs[rollCiv],
        map: maps[rollMap],
        victory: victories[rollVictory]
    };
}

function showResults(civ, map, victory) {
    ...
    divHeadline.innerText = getHeadline(victory.title, civ);
...
function getHeadline(victoryType, civName) {
    switch (victoryType) {
        case 'Domination':
